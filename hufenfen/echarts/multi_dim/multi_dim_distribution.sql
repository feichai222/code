...
GROUP BY 
    ...
    <#if (dimensions?? && dimensions?contains("game_name")) || game_name?? >
    <#if started == true>,</#if>game_name
    <#assign started = true>
    </#if>
    <#if (dimensions?? && dimensions?contains("version")) || version?? >
    <#if started == true>,</#if>version
    <#assign started = true>
    </#if>
    <#if (dimensions?? && dimensions?contains("product_type")) || product_type?? >
    <#if started == true>,</#if>product_type
    <#assign started = true>
    </#if>
    <#if (dimensions?? && dimensions?contains("region")) || region?? >
    <#if started == true>,</#if>region
    <#assign started = true>
    </#if>
    ORDER BY SUM(CAST(units AS DECIMAL(10,2))) DESC
